/**
 * Provider Badge - Atom Component
 *
 * Badge showing which OAuth provider is linked
...
export const ProviderBadge: FC<ProviderBadgeProps> = ({
  provider,
  size = 'md',
  iconOnly = false,
  className,
  ...
  return (
    ...
    >
      {config.icon(sizes.icon)}
      {!iconOnly && <span>{config.name}</span>}
    </span>
  );
};
